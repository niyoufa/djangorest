...
class Alipay(object):
    ...
    def verify_notify(self, **kw):
        ...
        if signmethod(kw) == sign[0].encode('utf-8'):
...
class WapAlipay(Alipay):
    ...
    def check_notify_remotely(self, **kw):
        if 'notify_data' in kw:
            notifydata = unquote(kw['notify_data'][0])
    ...
    def _generate_md5_notify_sign(self, kw):
        newpara = OrderedDict()
        newpara['service'] = kw['service'][0].encode('utf-8')
        newpara['v'] = kw['v'][0].encode('utf-8')
        newpara['sec_id'] = kw['sec_id'][0].encode('utf-8')
        newpara['notify_data'] = kw['notify_data'][0].encode('utf-8')
